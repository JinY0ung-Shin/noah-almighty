// One source for the icon set. `Icon.svelte` renders these inline; the
// imperative DOM helpers in `dom.ts` (code-block copy button, copied flash)
// build the same markup as a string. Keeping both on this map is what stops the
// two copies from drifting apart.
//
// Style contract for new entries: 24×24 viewBox, geometry only (no fill/stroke
// attributes — the wrapper supplies `fill="none"`, `stroke="currentColor"`,
// 2px round caps/joins).
export const ICON_PATHS: Record<string, string> = {
  ...
  image: '<rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="9" cy="9" r="2"/><path d="m21 15-3.6-3.6a2 2 0 0 0-2.8 0L5 21"/>',
  ...
  // chevron-down mirrored vertically. A real path, not a CSS-rotated
  // chevron-down: rotating the wrapper would also rotate any transform the
  // caller animates, and half the icon set is used inside spring-driven chrome.
  ...
  network: '<circle cx="12" cy="5" r="2.5"/><circle cx="5" cy="19" r="2.5"/><circle cx="19" cy="19" r="2.5"/><path d="m10.6 7.2-4.2 9.6M13.4 7.2l4.2 9.6"/>',
  "panel-left-close": '<rect x="3" y="3" width="18" height="18" rx="2"/><path d="M9 3v18"/><path d="m16 15-3-3 3-3"/>',
};

/**
 * The same icon as a standalone `<svg>` string, for the places that build DOM
 * imperatively instead of through a Svelte component. Mirrors the wrapper
 * attributes in `Icon.svelte` — change one, change both.
 */
